...
function markupForThumbSvgWithTooltips (classString, type, fillColor, stance) {
  ...
  const tooltips = {
    ...
    'STORED_SUPPORT': 'Endorsement entered & visible to voters',
    'POSSIBILITY_OPPOSE': 'Opposition entered, but not visible to voters yet',
    'STORED_OPPOSE': 'Opposition entered & visible to voters',
    'POSSIBILITY_INFO': 'Comment entered, but not visible to voters yet',
    'STORED_INFO': ' Comment entered & visible to voters',
    'DELETED': 'Candidate not found on this web page',
    'DEFAULT': 'Candidate not found in database'
  };

  // Determine the tooltip message based on the type.
  ...
  if (type === 'endorse') {
    ...
    fillColor = 'black';
    markup += "<path fill='" + fillColor + "' d='M1 21h22 1 21zm12-6h-2v-2h2v2zm0-4h-2v-4h2v4z'/>" +
    "<path d='M0 0h24v24H0z' fill='none'/>";
  }
