...
document.addEventListener("DOMContentLoaded", async function () {
  // This wastes about 1 ms for every open tab in the browser, that we are not going to highlight on
  ...
  $(document).ready(function () {
    console.log(
      "&&&&&&&&&&&&&&&&&&&&&&&&&&&& document ready in tabWordHighlighter    &&&&&&&&&&&&&&&&&&&&&&&&&&&&"
    );
  });
  ...
  timingLog(
    t0,
    t1,
    `DOMContentLoaded for tab ${window.location.href} took`,
    8.0
  );
  ...
  if (webAppHosts.includes(hostname)) {
    ...
    timingLog(
      t0,
      t1,
      `DOMContentLoaded for tab with voterDeviceId ${voterDeviceId} took`,
      8.0
    );
    if (voterDeviceId && voterDeviceId.length > 10) {
      ...
      timingLog(
        t0,
        t1,
        "DOMContentLoaded mergeToGlobalState voterDeviceId took",
        8.0
      );
    }
  }
...
function isInANonWeVoteIFrame() {
  return (
    (window.self !== window.top && $(".weVoteEndorsementFrame").length === 0) ||
...
function logInitialization(href, url) {
  debugFgLog("############ tabWordHighlighter url from state: ", url, href);
}

// Check if initialization should be skipped
function shouldSkipInitialization(href) {
  if (href === "about:blank" || href.includes("extension.html")) {
    debugFgLog("############ tabWordHighlighter NOT initializing for:", href);
    return true;
  }
...
// Handle initialization within an iFrame
async function handleIFrameInitialization() {
  // If not in our iFrame
  if (!isInOurIFrame() && !isInANonWeVoteIFrame()) {
    // Check to see if we are on the WebApp signin page, and capture the device id if signed in
    if (host.indexOf("wevote.us") > -1 || host.indexOf("localhost:3000") > -1) {
      ...
      if (voterDeviceIdFromPage.length) {
        getVoterDeviceId().then(voterDeviceId => {
          console.log(
            "initializeTabWordHighlighter voterDeviceId Value was ",
            voterDeviceId
          );
          const junk = voterDeviceId + "x|x";
          console.log(
            "initializeTabWordHighlighter voterDeviceId Value set to ",
            junk
          );
        });
      }
  ...
  // Handle initialization on the main page
  async function handleMainPageInitialization(host) {
    // Check to see if we are on the WebApp signin page, and capture the device id if signed in
    if (host.indexOf("wevote.us") > -1 || host.indexOf("localhost:3000") > -1) {
      const voterDeviceIdFromPage = getVoterDeviceIdFromWeVoteDomainPage();
      if (voterDeviceIdFromPage.length) {
  ...
  // Set up message listener for the extension
  ...
  async function onExtensionMessage(request, sender, sendResponse) {
    if (
      sender.id === "iehabmcmgpcollmfldiemagcdhnegmnk" || // New from Chrome store 7/12/23
      sender.id === "pmpmiggdjnjhdlhgpfcafbkghhcjocai" ||
      sender.id === "lfifjogjdncflocpmhfhhlflgndgkjdo" ||
      sender.id === "eofojjpbgfdogalmibgljcgdipkhoclc" ||
      sender.id === "ikoadphkdpbhakeghnjpepfgnodmonpk" ||
      ...
      sender.id === "ijckochdechhhdpdjfeglcjhmfhgeldn" || // Dale 7/12/23
      sender.id === "hpmgpbonilnlgbbmjllnpomdngadpkpk" ||
      sender.id === "ckdogheakehifgfhnldikhjgmhfcjhih" ||
      sender.id === "highlightthis@deboel.eu"
    ) {
      debugFgLog(
        "@@@@@@@@@@@ Message listener in tabWordHighlighter received: ",
        request.command,
        request.selection ? JSON.stringify(request.selection) : "",
        request.payload ? JSON.stringify(request.payload) : ""
      );
      ...
      debugFgLog("Message received in tabWordHighlighter: ", request);

      switch (request.command) {
        case "displayHighlightsForTabAndPossiblyEditPanes":
          handleDisplayHighlightsForTabAndPossiblyEditPanes(
            request,
            state,
            href
          );
        ...
        case "hardResetActiveTab":
          console.log(
            "hardResetActiveTab tabWordHighlighter location: ",
            location
          );
          location.reload();
          break;
        ...
        case "getMarkers":
          // Logic for getting markers
          sendResponse(highlightMarkers);
          break;

        case "ClearHighlights":
          // Logic for clearing highlights
          highlightMarkers = {};
          break;

        case "ReHighlight":
          // Logic for re-highlighting
          reHighlight(request.words);
          break;

        case "createEndorsement":
          console.log(
            "================================  received createEndorsement request.selection: ",
            request.selection
          );
          if (request.selection !== undefined) {
            await openSuggestionPopUp(request.selection);
            ...
            console.log(
              "================================  received createEndorsement with UNDEFINED request.selection"
            );
          }
          break;

        case "revealRight":
          // Logic for 'revealRight' command
          revealRightAction(request.selection);
          break;

        case "getStatusForActiveTab":
          // Logic for getting status for active tab
          const encodedHref = encodeURIComponent(location.href);
          const { orgName, organizationWeVoteId, organizationTwitterHandle } =
            state;
          debugFgLog("getStatusForActiveTab tabId: ", tabId, ", href: ", href);
        ...
        case "disableExtension":
          // Logic for disabling extension
          enableHighlightsForAllTabs(false);
          break;

        case "logFromPopup":
          // Logic for logging from popup
          console.log("popup: " + request.payload);
          break;

        case "updateForegroundForButtonChange":
          // Logic for updating foreground due to button change
          debugSwLog(
            'extWordHighlighter "updateForegroundForButtonChange" received from popup'
          );
          const { pdfURL, tabId, tabUrl } = request.payload;
          console.log(
            "updateForegroundForButtonChange: ",
            JSON.stringify(request.payload)
          );

          // Save the tabId and tabURL to global state
          console.log(
            "updateForegroundForButtonChange, saving tabId and tabURL to global state"
          );
          const stateUpdate = showHighlights
            ? { tabId: tabId, url: tabUrl }
            : { tabId: -1, url: tabUrl };
          await updateGlobalState(stateUpdate);

          // Sending a message to update the background state
          debugFgLog(
            "^^^^^^^^^^^^^^^^^^^ sendMessage updateBackgroundForButtonChange in isInANonWeVoteIFrame"
          );
          sendMessage({
            command: "updateBackgroundForButtonChange",
            showHighlights,
            ...
            console.log(
              "return from updateForegroundForButtonChange after send message updateBackgroundForButtonChange"
            );
          });
          break;
        ...
        default:
          console.error(
            "Unknown command received in tabWordHighlighter:",
            request.command
          );
          // Optionally, you can also send a response back indicating the unrecognized command
          if (sendResponse) {
  ...
  async function handleDisplayHighlightsForTabAndPossiblyEditPanes(
    request,
    state,
    href
  ) {
    debugFgLog(
      "------------------------------- ENTERING tabWordHighlighter > displayHighlightsForTabAndPossiblyEditPanes"
    );
    debugFgLog(
      "ENTERING tabWordHighlighter > displayHighlightsForTabAndPossiblyEditPanes showHighlights: ",
    ...
    if (href !== "about:blank" && state.showHighlights) {
      ...
      console.log("Processing selected tab href: ", href);
      displayHighlightingAndPossiblyEditor(
        state.showHighlights,
        state.showPanels,
        state.tabId
      );
      ...
      timingFgLog(
        performance.now(),
        t2,
        "displayHighlightsForTabAndPossiblyEditPanes processing took",
        8.0
      );
    }
  ...
  // Schedule a status check based on hardware concurrency
  function scheduleStatusCheck(href) {
    if (href !== "about:blank") {
      setTimeout(
        async () => {
          await sendGetStatus(); // Initial get status
        },
        navigator.hardwareConcurrency >= 10 ? 0 : 1500
      );
    }
  }

  // Main function to initialize tab word highlighter
  async function initializeTabWordHighlighter() {
    setupHeartbeat();
    const state = await getGlobalState();
    const { url } = state;
    ...
    logInitialization(href, url);
    if (shouldSkipInitialization(href)) return;

    await updateGlobalState({ neverHighlightOn: defaultNeverHighlightOn });

    if (isInOurIFrame()) {
      handleIFrameInitialization();
    } else if (!isInANonWeVoteIFrame()) {
      handleMainPageInitialization(host);
    }

    setupMessageListener();
    scheduleStatusCheck(href);
  }
      ...
      if (words[group].Enabled) {
        for (let word in words[group].Words) {
          ...
          if (words[group].Words[word]) {
            ...
            const oldWordElement = wordsArray.find(
              arrayItem => arrayItem.regex === reg
            );
            if (oldWordElement) {
              const exactDupe =
                JSON.stringify(oldWordElement) ===
                JSON.stringify(newWordsElement);
              if (exactDupe) {
  ...
  function reHighlight(words) {
    ...
    reHighlightDebug &&
      debugFgLog("ENTERING function reHighlight(words), words:", words);
    addWordsToWordsArray(words);
    reHighlightDebug &&
      debugFgLog(
        "reHighlight before findWords --------------------------- namesToIds: ",
        namesToIds
      );
  ...
  async function getVoterDeviceIdFromWeVoteDomainPage() {
    ...
    try {
      ...
      if (id && id.length > 5) {
        debugFgLog(
          "=======  voter_device_id captured from a wevote page, id: " + id
        );
        await mergeToGlobalState({ voterDeviceId: id });
        debugFgLog(
          "======= voterDeviceId was merged mergeToGlobalState from a wevote page, id: " +
            id
        );
      } else {
  ...
  async function sendGetStatus() {
    ...
    for (let i = 0; i < neverHighlightOn.length; i++) {
      ...
      if (window.location.hostname.match(reg)) {
        debugFgLog(
          "sendGetStatus found a neverHighlightOn match: ",
    ...
    sendMessage(
      { command: "getStatus", tabURL: window.location.href },
      async function (response) {
        debugFgLog(
        ...
        if (lastError) {
          debugFgLog(
            'chrome.runtime.sendMessage("getStatus")',
            lastError.message
          );
          return;
        }
        ...
        if (url && !url.includes(window.location.host)) {
          debugFgLog(
            "sendMessage for getStatus aborted on page ",
            window.location.host,
            " returned url: ",
            url,
            ", with probably wrong tab id: ",
            tabId
          );
          return;
        }
        ...
        debugFgLog(
          "sendGetStatus -- Testing for correct tab -- correct: ",
          highlightThisTab,
          ", this tabID: ",
          tabId,
          ", tabIdFromState:",
  ...
  async function closeIFrameDialog() {
    ...
    if (isInOurIFrame()) {
      // if in an iframe
      debugFgLog(
        "With editors displayed, and the endorsement page in an iFrame, the modal containing an iFrame to the webapp has closed.  Evaluating the need to update the PositionsPanel,state "
      );
      ...
      debugFgLog(
        "dialog containing iFrame has closed, either without the editor displayed, or for newly discovered positions, ie right click on highlighed position"
      );
  ...
  function createWediv() {
    debugFgLog(
      "---------------------------------------- inserting wediv for the dialog into the top of the body"
    );
    ...
    const css =
      "#wediv{position:absolute;z-index:10000;background-color:#000;text-align:center;border:1px solid #d3d3d3;box-shadow:10px 10px 5px 0 rgba(0,0,0,.4);height:640px;}#wedivheader{cursor:move;z-index:10;background-color:#2e3c5d;color:#fff;height:30px}#weIFrame{width:450px;height:568px;border-width:0;border:none}#wetitle{float:left;margin-left:8px;margin-top:2px}.weclose{height:10px;width:10px;float:right;margin-right:16px;background-color:#2e3c5d;color:#fff;border:none;font-weight:bolder;font-stretch:extra-expanded;font-size:12pt}.highlight{padding:1px;box-shadow:#e5e5e5 1px 1px;border-radius:3px;-webkit-print-color-adjust:exact;background-color:#ff6;color:#000;font-style:inherit}";
    ...
    window.addEventListener(
      ...
      function (e) {
        if (e.data === "closeIFrameDialog") {
          ...
          debugFgLog(
            "tabWordHighlighter (create new) receiving close message from iFrame: ",
            e.data
          );
  ...
  async function getWordsThenStartHighlighting() {
    ...
    debugFgLog(
      'ENTERING tabWordHighlighter > getWordsThenStartHighlighting,  About to call "getWords"'
    );
    const state = await getGlobalState();
    const { tabId } = state;
    ...
    debugFgLog(
      "^^^^^^^^^^^^^^^^^^^ sendMessage getWords #1 in getWordsThenStartHighlighting"
    );
    sendMessage(
      {
        command: "getWords",
      ...
      function (response) {
        if (!response) {
          debugFgLog(
            'chrome runtime sendMessage("getWords") did not receive a response'
          );
          return;
        }
        ...
        getWordsThenStartHighlightingDebug &&
          debugFgLog(
            "Received response getWordsThenStartHighlighting() URL: ",
            url
          );

        if (lastError) {
          debugFgLog(
            'chrome runtime sendMessage("getWords")',
            lastError.message
          );
          return;
        }
        getWordsThenStartHighlightingDebug &&
          debugFgLog("got words response: ", response);
        ...
        timingFgLog(
          t1,
          t2,
          'in getWordsThenStartHighlighting, retrieving "getWords" highlighting rules took',
          8.0
        );
    ...
    debugFgLog(
      "In getWordsThenStartHighlighting AFTER IN CODE setTimer for 10 SECOND DELAY HACK"
    ); // 6/27/23 ??? does this still make sense
  }
  ...
  async function highlightLoop() {
    ...
    if (highlighterLooping) {
      ...
      console.log(
        "-------- calling findWords outside of the highlightLoop ----------- "
      );
      await findWords();
    } else {
      ...
      let highlightLoopIntervalId = setInterval(async function () {
        try {
          ...
          if (!showHighlights && !showPanels) {
            console.log(
              "-------- CLEARING INTERVAL in highlightLoop ----------- ",
              i,
              showHighlights,
              showPanels,
              highlightLoopIntervalId
            );
  ...
  function addToPeakDefaultsIfYellow(marker, yellows) {
    if (marker.color === "#ff6") {
      // Re-capitalize name
      const capitalizedName = marker.word.replace(
        /(^\w{1})|(\s+\w{1})/g,
  ...
  function logDebugForTwoCandidates() {
    for (let i = 0; i < wordsArray.length; i++) {
      if (wordsArray[i].word === "pete aguilar") {
        ...
        console.log(
          '~~~~~~~~~~~~~~~~~~~~~ entry to findWords() wordsArray[i].word === "pete aguilar", wordsArray[i].Color: ',
          c,
          i
        );
      }
      if (wordsArray[i].word === "jake auchincloss") {
        ...
        if (wordsArray[i].Color === "#ff6") c = "yellow";
        console.log(
          '~~~~~~~~~~~~~~~~~~~~~ entry to findWords() wordsArray[i].word === "jake auchincloss", wordsArray[i].Color: ',
          c,
          i
        );
      }
    ...
    if (Object.keys(wordsArray).length > 0) {
      ...
      setTimeout(async function () {
        ...
        if (highlightsObj.numberOfHighlights > 0) {
          ...
          for (let marker in highlightMarkers) {
            if (
              markerPositions.indexOf(highlightMarkers[marker].offset) === -1
            ) {
          ...
          for (let i = 0; i < len; i++) {
            ...
            if (!uniqueNameMatches.includes(word)) {
              ...
              addElementToPositions(state.positions, {
                ballot_item_name: word,
              });
            }
          }
          if (yellows.length) {
          ...
          try {
            console.log(
              "sending showHighlightsCount in findWords (762), count = " +
                count.toString()
            ); // always log this
            noResponseSendMessageWrapper(
              sendMessage,
              {
            ...
            console.log(
              "sending showHighlightsCount in findWords (777), after noResponseSendMessageWrapper"
            );
          } catch (e) {
          ...
          try {
            ...
            if (count > 0) {
              console.log(
                "sending showHighlightsCount in findWords (782), based on positions: ",
                count.toString()
              ); // always log this
              noResponseSendMessageWrapper(
                sendMessage,
  ...
  function convertV2onClickToV3() {
    // 4/5/23: Removed the ".each" replacement of setModal that had been pre-written into every wrapped highlight markup, and replace it with a onClick made on the fly.
  ...
  function revealRightAction(selection) {
    const nameMatchElement = $("#sideArea")
      .contents()
      .find(":contains(" + selection + "):last");
