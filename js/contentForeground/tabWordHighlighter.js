...
async function initializeTabWordHighlighter() {
  ...
  if (isInOurIFrame()) {
    debugFgLog(
      "^^^^^^^^^^^^^^^^^^^ sendMessage myTabId in initializeTabWordHighlighter"
    );
    sendMessage(
      {
        command: "myTabId",
      ...
      async function (response) {
        if (lastError) {
          debugFgLog(
            ' chrome.runtime.sendMessage("myTabId")',
            lastError.message
          );
        }
        ...
        debugFgLog(
          "tabWordHighlighter initializeTabWordHighlighter tab.id: " + tabId
        );
      }
    );
  }
  ...
  if (!isInOurIFrame() && !isInANonWeVoteIFrame()) {
    ...
    onMessage.addListener(
      ...
      async function (request, sender, sendResponse) {
        ...
        tabWordHighlighterListenerDebug &&
          debugFgLog(
            "message listener in tabWordHighlighter got a message: ",
            request,
            sender,
            sendResponse
          );
        ...
        if (
          ...
          if (
            request.command === "displayHighlightsForTabAndPossiblyEditPanes"
          ) {
            if (href.toLowerCase().endsWith(".pdf")) {
              debugFgLog(
                "displayHighlightsForTabAndPossiblyEditPanes skipping PDF file"
              );
              return false;
            }
            const t1 = performance.now();
            ...
            if (!showHighlights) {
              debugFgLog(
                "displayHighlightsForTabAndPossiblyEditPanes (before reload)"
              );
            ...
            console.error(
              "tabWordHighlighter in chrome.runtime.onMessage.addListener received unknown command: " +
                request.command
            );
            return false;
          }
  ...
  if (href !== "about:blank") {
    ...
    ); // Timers like this are the last resort
  }
}
