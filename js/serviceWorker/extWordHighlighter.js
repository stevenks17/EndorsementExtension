  ...
  for (let i = 0; i < highlightsList.length; i++) {
    ...
    if (stance.length === 0) {
      ...
      if (highlight.display === display && highlight.stance === stance) {
        ...
        display === "POSSIBILITY" &&
        highlight.position_stance === stance
      ) {
        // On processing updated possibility positions after editing existing position from endorsement site page in paneled layour
  ...
  for (let i = 0; i < ballotItemHighlights.length; i++) {
    ...
    if (name && name.length > 2) {
      ...
      if (match && match.display) {
        ...
        overlayOrganizationPositionsDebug &&
          debugSwLog(
            "For " +
              name +
              " overwriting to " +
...
function overlayOrganizationPositions(voterGuideHighlights) {
  ...
  overlayOrganizationPositionsDebug &&
    debugSwLog("ENTERING extWordHighlighter > overlayOrganizationPositions");
  aliasNames = [];
  nameToIdMap = {};
...
async function initializeEndorsementDetectionList(voterGuideHighlights) {
  debugSwLog(
    "ENTERING initializePositionsArray, needed for non-paned mode Edit/Add decision for the modal pop up"
  );
  const nameArray = [];
  ...
  if (currentEndorsementsArray.length) {
    debugStorage(
      "---------------------------- initializeEndorsementDetectionList, before save ---- currentEndorsementsArray: ",
      currentEndorsementsArray
    );
...
function initializeHighlightsData(
  ballotItemHighlights,
  voterGuideHighlights,
  neverHighLightOnLocal
) {
  const initializeHighlightsDataDebug = true;
  initializeHighlightsDataDebug &&
    debugSwLog(
      "ENTERING extWord > initializeHighlightsData, ballotItemHighlights.length:",
      ballotItemHighlights.length
    );
...
async function initializeVoterGuideHighlightsData(
  tabId,
  voterGuideHighlights,
  neverHighLightOnLocal
) {
  const initializeVoterGuideHighlightsDataDebug = true;
  initializeVoterGuideHighlightsDataDebug &&
    debugSwLog(
      "ENTERING extWordHighlighter > initializeVoterGuideHighlightsData"
    );
  overlayOrganizationPositions(voterGuideHighlights);
  await initializeEndorsementDetectionList(voterGuideHighlights);

  HighlightsData.Version = "12";
  if (neverHighLightOnLocal !== null) {
    // If we receive a null, only overwrite (the already existing) HighlightsData.Groups (as a result of edit in paned pop-up dialog)
    HighlightsData.neverHighlightOn = preProcessNeverList(
      neverHighLightOnLocal
    );
    HighlightsData.PrintHighlights = true;
    let today = new Date();
  ...
  for (let groupName in groupNames) {
    ...
    debugSwLog(
      "initializeVoterGuideHighlightsData groupName: ",
      groupName,
      group
    );
    HighlightsData.Groups.push(groupName, group);
...
function getColor(typeStance, foreground) {
  switch (typeStance) {
    case "POSSIBILITY_SUPPORT":
      return foreground
        ? colors.POSS_SUPPORT_FOREGROUND
        : colors.POSS_SUPPORT_BACKGROUND;
    case "POSSIBILITY_OPPOSE":
      return foreground
        ? colors.POSS_OPPOSE_FOREGROUND
        : colors.POSS_OPPOSE_BACKGROUND;
    case "POSSIBILITY_INFO":
      return foreground
        ? colors.POSS_INFO_FOREGROUND
        : colors.POSS_INFO_BACKGROUND;
    case "STORED_SUPPORT":
      return foreground
        ? colors.STORED_SUPPORT_FOREGROUND
        : colors.STORED_SUPPORT_BACKGROUND;
    case "STORED_OPPOSE":
      return foreground
        ? colors.STORED_OPPOSE_FOREGROUND
        : colors.STORED_OPPOSE_BACKGROUND;
    case "STORED_INFO":
      return foreground
        ? colors.STORED_INFO_FOREGROUND
        : colors.STORED_INFO_BACKGROUND;
    case "DELETED":
...
function getIcon(typeStance) {
  switch (typeStance) {
    case "POSSIBILITY_SUPPORT":
      return markupForThumbSvg(
        "thumbIconSVGContent",
        "endorse",
        getColor(typeStance, true)
      );
    case "POSSIBILITY_OPPOSE":
      return markupForThumbSvg(
        "thumbIconSVGContent",
...
function createNewTabsHighlightedElement(tabId, url) {
  ...
  debugSwLog(
    "^^^^^^^^ createNewTabsHighlightedElement before, for tab:",
    tabId,
    tabsHighlighted[tabId]
  );
  ...
  debugSwLog(
    "^^^^^^^^ createNewTabsHighlightedElement after, for tab:",
    tabId,
    tabsHighlighted[tabId]
  );
...
function updateContextMenu(inUrl, tabId) {
  ...
  if (inUrl && noContextMenu.indexOf(inUrl) === -1) {
    ...
    sortedByModified.sort(function (a, b) {
      return b[1] - a[1];
    });
        ...
        create(
          ...
          () => {
            ...
            if (myError) {
              ...
              debugFgLog(
                "----------------------------------- contextMenuCreated myError during create: ",
                myError.message
              );
            }
          }
...
function hardResetActiveTab(tabId) {
  ...
  debugFgLog(
    "^^^^^^^^^^^^^^^^^^^ sendMessage tabId weird parameters! in hardResetActiveTab"
  );
  sendMessage(
    tabId,
    {
    ...
    function () {
      if (lastError) {
        debugSwLog(
          ' chrome.runtime.sendMessage("hardResetActiveTab")',
          lastError.message
        );
      }
...
function setEnableForActiveTab(showHighlights, showEditor, tabId, tabUrl) {
  ...
  const {
    tabs: { sendMessage, lastError },
  } = chrome;
  setEnableForActiveTabDebug &&
    debugSwLog(
      "POPUP BUTTON CLICKED: enabling highlights on active tab ",
      tabId,
      ", showEditor: ",
      showEditor,
      ", showHighlights:",
  ...
  debugSwLog(
    "sendMessage displayHighlightsForTabAndPossiblyEditPanes tabId:",
    tabId,
    ", showHighlights: ",
    showHighlights,
    ", showEditor",
  ...
  debugFgLog(
    "^^^^^^^^^^^^^^^^^^^ sendMessage displayHighlightsForTabAndPossiblyEditPanes ext in setEnableForActiveTab"
  );
  sendMessage(
    tabId,
    {
    ...
    function (result) {
      if (lastError) {
        debugSwLog(
          ' chrome.runtime.sendMessage("displayHighlightsForTabAndPossiblyEditPanes")',
          lastError.message
        );
      }
      debugSwLog(
        "RESPONSE sendMessage displayHighlightsForTabAndPossiblyEditPanes tabId:",
        tabId
      );
      debugSwLog(
        "on click highlight this tab or edit this tab, response received from displayHighlightsForTabAndPossiblyEditPanes ",
        result
      );
    }
  );
...
chrome.tabs.onActivated.addListener(function (tabId) {
  chrome.tabs.query(
    ...
    async function (tabs) {
      ...
      debugSwLog(
        "XXXXXX getGlobalState in tabs onactivated 2.5",
        state.tabId,
        state.url
      );
      ...
      if (tabs.length) {
        debugSwLog(
          "XXXXXX decompose tabs onactivated raw tabs: ",
          JSON.stringify(tabs)
        );
        const {
          0: { id, url },
        ...
        if (url !== "chrome://extensions/" && id === state.tabId) {
          ...
          debugSwLog(
            'XXXXXX chrome.tabs.onActivated chrome.tabs.query triggered for "chrome://extensions/" or for a tab that is not supposed to be highlighted so it was ignored: ',
            url,
            id,
            state.tabId
          );
        ...
        debugSwLog(
          "chrome.tabs.onActivated.addListener found no currentWindow for tabId: " +
            tabId
        );
      }
    }
...
chrome.contextMenus.onClicked.addListener(function (info, tab) {
  ...
  console.log(
    "================================ chrome.contextMenus.onClicked text: " +
      info.selectionText
  );
  console.log(
    "================================ chrome.contextMenus.onClicked obj: " +
      JSON.stringify(info)
  );
  console.log(
    "================================ chrome.contextMenus.onClicked info.menuItemId: " +
      info.menuItemId +
      ", " +
      info.menuItemId ===
      "idContextMenuCreateNew"
  );
  console.log(
    "================================ chrome.contextMenus.onClicked info.menuItemId.indexOf('idContextMenuCreateNew'): " +
      info.menuItemId.indexOf("idContextMenuCreateNew")
  );

  if (info.menuItemId.indexOf("idContextMenuCreateNew") > -1) {
    console.log(
      "================================ chrome.contextMenus.onClicked sending createEndorsement"
    );
    debugFgLog(
      "^^^^^^^^^^^^^^^^^^^ sendMessage createEndorsement ext in onClicked idContextMenuCreateNew"
    );
    sendMessage(tab.id, {
    ...
    debugFgLog(
      "^^^^^^^^^^^^^^^^^^^ sendMessage revealRight ext in onClicked idContextMenuRevealRight"
    );
    sendMessage(
      tab.id,
      {
      ...
      function (result) {
        if (lastError) {
          debugSwLog(
            ' chrome.runtime.sendMessage("revealRight")',
            lastError.message
          );
        }
        debugSwLog(
          "contextMenus on click, response received to revealRight ",
          result
        );
      }
    );
    ...
    if (
      HighlightsData.Groups[groupName].Words.indexOf(info.selectionText) > -1
    ) {
    ...
    query({ active: true, currentWindow: true }, function (tabs) {
      debugFgLog(
        "^^^^^^^^^^^^^^^^^^^ sendMessage ScrollHighlight ext in onClicked Highlight"
      );
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  debugSwLog(
    "MESSAGING: message received -- " + request.command + ": ",
    request,
    sender,
    sender.tab.id
  );
  ...
  if (message.command === "heartbeat") {
    console.log("Heartbeat received from", sender.id);
    // Respond back if necessary, or simply acknowledge the heartbeat
  ...
  if (request.command === "getTopMenuData") {
    ...
    getHighlightsListsFromApiServer(
      request.url,
      request.voterDeviceId,
      request.tabId,
      request.doReHighlight,
      sendResponse,
      showVoterGuideHighlights,
      showCandidateOptionsHighlights
    );
    ...
    debugSwLog(
      "MESSAGING: voterGuidePossibilityPositionSave message received",
      request,
    ...
    voterGuidePossibilitySave(
      request.organizationWeVoteId,
      request.voterGuidePossibilityId,
    ...
    sendResponse({
      command: request.command,
      success: setPrintHighlights(request.state),
    });
  } else if (request.command === "addGroup") {
    debugSwLog(
      "MESSAGING: XXXXXXXXX NO LONGER CALLED XXXXXXXXXXX addGroup message received",
      request,
      sender,
      sender.tab.id
    );
    sendResponse({
      ...
      success: addGroup(
        ...
        request.regex,
        request.showInEditableFields
      ),
    });
  } else if (request.command === "removeWord") {
    sendResponse({
      command: request.command,
      success: removeWord(request.word),
    });
  } else if (request.command === "beep") {
    document.body.innerHTML +=
      '<audio src="../../beep.wav" autoplay="autoplay"/>';
    ...
    sendResponse({
      command: request.command,
      success: flipGroup(request.group, request.action),
    });
  } else if (request.command === "deleteGroup") {
    sendResponse({
      command: request.command,
      success: deleteGroup(request.group),
    });
    ...
    sendResponse({
      command: request.command,
      success: addWords(request.words),
    });
    ...
    const tabId = sender.tab.id;
    debugSwLog(
      'extWordHighlighter "closeDialogAndUpdatePositionsPanel" received from tab: ' +
        tabId
    );
  } else if (request.command === "hardResetActiveTab") {
    const tabId = sender.tab.id;
    debugSwLog(
      'extWordHighlighter "hardResetActiveTab" received from tab: ' + tabId
    );
    hardResetActiveTab(tabId);
  } else if (request.command === "updateBackgroundForButtonChange") {
    debugSwLog(
      'extWordHighlighter "updateBackgroundForButtonChange" received from popup for tab: ',
      request.tabId
    );
    handleButtonStateChange(
      request.showHighlights,
      request.showPanels,
      request.pdfURL,
      request.tabId,
      request.tabUrl
    ...
    console.error(
      "extWordHighlighter received unknown command : " + request.command
    );
  }
  return true;
});
...
function getThisTabsStatus(tabURL, sendResponse) {
  ...
  debugSwLog(
    "extWordHighlighter.getThisTabsStatus () {() called for url",
    tabURL
  );
  ...
  query({}, async function (tabs) {
    ...
    if (tab !== {}) {
      ...
      if (!tabsHighlighted[tabId]) {
        debugSwLog(
          "extWordHighlighter.getThisTabsStatus created a NEW tabsHighlighted entry for id ",
          tabId,
          url
        );
        status = createNewTabsHighlightedElement(tabId, url);
        ...
        debugSwLog(
          "extWordHighlighter.getThisTabsStatus found an EXISTING tabsHighlighted entry for id ",
          tabId,
          url
        );
      }
      ...
      debugSwLog(
        "extWordHighlighter.getThisTabsStatus query returned no tabs, and DID NOT FIND AN EXISTING tabsHighlighted SO WE created a new tabInfoObj entry with a tabId of -1"
      );
    }
    ...
    status = {
      ...
      showEditor,
      showHighlights,
      highlighterEnabled: showHighlights,
    };
...
function requestReHighlight(tabId, url) {
  ...
  if (tabId && tabId > 0) {
    debugSwLog(
      "In extWordHighlighter.requestReHighlight, ReHighlight WAS sent -- for tabId:",
      tabId
    );
    debugFgLog(
      "^^^^^^^^^^^^^^^^^^^ sendMessage ReHighlight ext in requestReHighlight"
    );
    sendMessage(
      tabId,
      { command: "ReHighlight", words: getWordsBackground(url, tabId) },
      function () {
        debugSwLog(
          lastError
            ? `requestReHighlight lastError ${lastError.message}`
            : "requestReHighlight returned"
        );
      }
    );
  } else {
    debugSwLog(
      "In extWordHighlighter.requestReHighlight, ReHighlight could not be sent -- missing tabId:",
      tabId
    );
  }
}
...
function getWordsBackground(inUrl, tabId) {
  ...
  if (
    !urlToHighlight.startsWith("https://") ||
    !urlToHighlight.startsWith("http://")
  ) {
    urlToHighlight = "https://" + urlToHighlight;
  }

  debugSwLog(
    "ENTRY to extWordHighlighter.getWordsBackground urlToHighlight: " +
      urlToHighlight
  );
  let result = {};
  for (let neverShowOn in HighlightsData.neverHighlightOn) {
    if (
      urlToHighlight.match(
        globStringToRegex(HighlightsData.neverHighlightOn[neverShowOn])
      )
    ) {
      return result;
    }
    ...
    if (HighlightsData.Groups[highlightData].Enabled) {
      if (HighlightsData.Groups[highlightData].ShowOn.length === 0) {
        ...
        for (let showOn in HighlightsData.Groups[highlightData].ShowOn) {
          if (
            urlToHighlight.match(
              globStringToRegex(
                HighlightsData.Groups[highlightData].ShowOn[showOn]
              )
            )
          ) {
            returnHighlight = true;
      ...
      for (let dontShowOn in HighlightsData.Groups[highlightData].DontShowOn) {
        if (
          urlToHighlight.match(
            globStringToRegex(
              HighlightsData.Groups[highlightData].DontShowOn[dontShowOn]
            )
          )
        ) {
  ...
  debugSwLog(
    "Exit from extWordHighlighter.getWordsBackground: " + urlToHighlight
  );
  return result;
}
  ...
  query({ active: true, currentWindow: true }, function (tabs) {
    ...
    sendMessage(tabs[0].id, { command: "getMarkers" }, function (result) {
      if (result) {
        if (lastError) {
          debugSwLog(
            ' chrome.runtime.sendMessage("getMarkers")',
            lastError.message
          );
        }
        return result;
...
function addGroup(
  ...
  remoteConfig,
  regex,
  showInEditableFields
) {
  for (let word in inWords) {
    inWords[word] = inWords[word].replace(/(\r\n|\n|\r)/gm, "");
  }
  HighlightsData.Groups[inGroup] = {
    Color: color,
    Fcolor: fcolor,
    Enabled: true,
    ShowOn: showon,
    DontShowOn: dontshowon,
    FindWords: findwords,
    Type: groupType,
...
function dumpTabStatus() {
  ...
  for (let key in tabsHighlighted) {
    if (!tabsHighlighted.hasOwnProperty(key)) continue;
    const { url, tabId, showEditor, showHighlights, twitterHandle } =
      tabsHighlighted[key];
    debugSwLog(
      "DUMP dumpTabStatus key:",
      key,
      ", tabId:",
      tabId,
      ", showEditor:",
...
function getWeVoteTabs() {
  ...
  getAllInWindow(null, function (tabs) {
    ...
    for (let i = 0; i < tabs.length; i++) {
      ...
      if (
        url.includes("https://wevote.us/") ||
        url.includes("https://quality.wevote.us/") ||
    ...
    create({ url: htmlURL }, () => {
      ...
      onUpdated.addListener(async function (newTabId, info) {
        await updateGlobalState({
          ...
          debugSwLog(
            "reloadPdfTabAsHTML in create()  onUpdated   newTabId: " +
              newTabId +
              ", info: " +
              JSON.stringify(info)
          );
...
function handleButtonStateChange(
  ...
  if (pdfURL) {
    debugSwLog(
      "extWordHighlighter.handleButtonStateChange enabling highlights on active tab FOR A PDF -- popup.js tab.id: ",
      tabId,
      pdfURL
    );
